...
export const MainLayout: React.FC = () => {
    ...
    const handleSaveCharacter = async (character: Character) => {
        ...
        // If new or no keys, generate keys
        ...
        // Sign character with User's Master Key
        if (appData.userKeys) {
        ...
        // Migrate deprecated ragSources to Knowledge Base if any
        if (updatedCharacter.ragSources && updatedCharacter.ragSources.length > 0) {
            ...
            updatedCharacter.ragSources = []; // Clear deprecated field
    ...
    const handlePermanentlyDeleteCharacter = (characterId: string) => {
        ...
        setConfirmationRequest({
            ...
            onConfirm: () => {
                ...
                // Clean up linked documents if they are not used by anyone else? 
                // For now, keep documents in library to be safe.
        ...
        reader.onload = async (e) => {
            ...
            try {
                ...
                if (importResult) {
                    ...
                    // --- Identity Verification & Update Logic ---
                    ...
                    if (existingCharacterIndex !== -1) {
                        ...
                        logger.log(`Character ID ${character.id} already exists. Checking ownership...`);
                        
                        // NOTE: If we are importing, we assume the file provided is the desired state.
                        // However, we must preserve our private keys if the import doesn't have them.
                        // Nexus export strips private keys, so importing a Nexus card usually means updating definitions.
                        
                        const mergedCharacter = {
                            ...character,
                            // Preserve local keys if import doesn't have them (likely public card)
                            keys: character.keys || existingChar.keys,
                            // Preserve our user signature if we signed it and it matches (complex, simple merge for now)
                            userPublicKeyJwk: character.userPublicKeyJwk || existingChar.userPublicKeyJwk,
                            signature: character.signature || existingChar.signature
                        };

                        updatedCharacters[existingCharacterIndex] = mergedCharacter;
                        alertMessage = `Character "${character.name}" updated successfully.`;
                    } else {
                        updatedCharacters.push(character);
                        alertMessage = `Character "${character.name}" imported successfully.`;
                    }
                    ...
                    logger.log(`Import completed for: ${character.name}`);
                    alert(alertMessage);
                    return;
                }
                ...
                if (data.id && Array.isArray(data.messages) && Array.isArray(data.characterIds)) {
                    ...
                    if (exists) {
                        if (!window.confirm(`Chat session "${data.name}" already exists. Overwrite?`)) {
                            return;
                        }
                        const updatedSessions = appData.chatSessions.map(s => s.id === data.id ? data as ChatSession : s);
                        const updatedData = { ...appData, chatSessions: updatedSessions };
                        setAppData(updatedData);
                        await persistData(updatedData);
                        alert(`Chat "${data.name}" updated.`);
                    } else {
                        const newSession: ChatSession = { ...data }; // Use ID from file to maintain link
        ...
        switch (activeView) {
            case 'character-form':
                return <CharacterForm 
                    ...
                    availableDocuments={appData.knowledgeBase || []}
                />;
            case 'plugins':
